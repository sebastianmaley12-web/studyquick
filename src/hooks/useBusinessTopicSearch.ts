...
import { BUSINESS_RESOURCES, type BusinessResource } from '../lib/content/business'
import { getBusinessTopicSearchIndex } from '../lib/businessSearchIndex'
import { requestReveal } from '../lib/searchRevealBus'

/**
 * Cross-tab search for a Business Studies topic — a parallel copy of
 * useTopicSearch.ts/useHmsTopicSearch.ts rather than a generalized shared
 * hook, since each hardcodes a different resource type and route path. See
 * useTopicSearch.ts's doc comment for why the hit tracking works the way it
 * does (never caching <mark> node references).
 */
export function useBusinessTopicSearch(
  topicId: string,
  ...
  function runSearch(q: string) {
    ...
    const index = getBusinessTopicSearchIndex(topicId)
    const lc = trimmed.toLowerCase()
    const hasMatch = (r: BusinessResource) => index[r].toLowerCase().includes(lc)
    ...
    const target = BUSINESS_RESOURCES.find(hasMatch)
    if (!target) {
      setCommittedQuery(trimmed)
      setMeta('No matches')
      return
    ...
    setCommittedQuery(trimmed)
    navigate(`/subjects/business/${topicId}/${target}`)
  }
