...
import { MATHS_RESOURCES, type MathsResource, type MathsTopic } from '../lib/content/maths'
import { getMathsTopicSearchIndex } from '../lib/mathsSearchIndex'

/**
 * Search for a Maths topic's "Key Facts & Formulae" tab — a parallel copy
 * of useLegalTopicSearch.ts/useHmsTopicSearch.ts/useBusinessTopicSearch.ts,
 * adapted for the fact that Maths only has one searchable resource (see
 * mathsSearchIndex.ts for why "Test yourself" isn't included).
 */
export function useMathsTopicSearch(
  topic: MathsTopic,
  resource: MathsResource,
  ...
  function jumpToHit(index: number) {
    ...
    hits[wrapped].scrollIntoView({ behavior: 'smooth', block: 'center' })
  ...
  function runSearch(q: string) {
    ...
    const index = getMathsTopicSearchIndex(topic)
    const lc = trimmed.toLowerCase()
    const hasMatch = (r: MathsResource) => index[r].toLowerCase().includes(lc)
    ...
    const target = MATHS_RESOURCES.find(hasMatch)
    if (!target) {
      setCommittedQuery(trimmed)
      setMeta('No matches')
      return
    ...
    setCommittedQuery(trimmed)
    navigate(`/subjects/maths/${topic.slug}/${target}`)
  }
